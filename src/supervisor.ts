...
export const make = (
  ...
  Effect.gen(function* () {
    ...
    const logStoreFailure = (operation: string) => (error: Store.StoreError) =>
      Effect.logError(`opencode-schedule-prompt: ${operation}: ${error.display}`)
    ...
    const drop = (id: WaitID) =>
      store.remove(id).pipe(Effect.catch(logStoreFailure("could not remove wait")))
      ...
      Effect.gen(function* () {
        ...
        if (!alive) {
          yield* Effect.logInfo(
            `opencode-schedule-prompt: dropping ${wait.id}, session ${wait.sessionID} is gone`,
          )
          yield* drop(wait.id)
          return
        }
        ...
        if (retry === undefined) {
          yield* Effect.logError(
            `opencode-schedule-prompt: giving up on ${wait.id} after ${wait.attempts} attempts`,
          )
          yield* drop(wait.id)
          return
        }
        ...
        yield* Effect.logWarning(
          `opencode-schedule-prompt: delivery of ${wait.id} failed, re-arming attempt ${retry.attempts}`,
        )
    ...
    const arm = (wait: Wait): Effect.Effect<void> =>
      Effect.gen(function* () {
        ...
        if (delay > 0 && wait.firesAt < now) {
          yield* Effect.logInfo(`opencode-schedule-prompt: ${wait.id} is overdue, firing now`)
        }
