...
describe("options.resolve", () => {
  test("defaults to queue delivery for {}", () => {
    expect(resolveSync({})).toEqual({ delivery: "queue" })
  })

  test("defaults to queue delivery for undefined", () => {
    expect(resolveSync(undefined)).toEqual({ delivery: "queue" })
  })
  ...
  test("honours an explicit steer delivery", () => {
    expect(resolveSync({ delivery: "steer" })).toEqual({ delivery: "steer" })
  })

  test("honours an explicit queue delivery", () => {
    expect(resolveSync({ delivery: "queue" })).toEqual({ delivery: "queue" })
  })

  test("falls back to the default for an unknown delivery literal", () => {
    expect(resolveSync({ delivery: "nope" })).toEqual({ delivery: "queue" })
  })

  test("falls back to the default for a wrongly typed delivery", () => {
    expect(resolveSync({ delivery: 5 })).toEqual({ delivery: "queue" })
  })

  test("falls back to the default for a non-object input", () => {
    expect(resolveSync("not an object")).toEqual({ delivery: "queue" })
  })

  test("falls back to the default for null", () => {
    expect(resolveSync(null)).toEqual({ delivery: "queue" })
  })

  test("falls back to the default when extra unknown fields are present", () => {
    expect(resolveSync({ delivery: "steer", extra: true })).toEqual({ delivery: "steer" })
  })
})
